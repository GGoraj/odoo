...
class CustomerDepositInvoiceAttemptErrorMessage(models.Model):
    ...
    def _post(self, soft=True):
        ...
        for move in self:
            ...
            if not move.line_ids.filtered(lambda line: line.display_type not in ('line_section', 'line_note')):
                raise UserError(_('Creating an invoice for cash deposit is not allowed.\nThere is no products to invoice. '))
